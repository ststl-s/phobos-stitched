...
void __fastcall TechnoExt::ExtData::UpdateTypeData(const TechnoTypeClass* currentType)
{
	...
	TechnoTypeClass* pType = this->TypeExtData->OwnerObject();
	TechnoExt::CheckNewWeapons(pThis, pType, this->TypeExtData);
}
...
void TechnoExt::ExtData::SelectSpecialWeapon(AbstractClass* pTarget)
{
	const auto pThis = this->OwnerObject();
	const auto pExt = this;
	const auto pTypeExt = pExt->TypeExtData;

	if (!pThis || !pExt || !pTypeExt || !pTypeExt->UseWeapons.Get())
		return;

	if (const auto pTechno = abstract_cast<TechnoClass*>(pTarget))
	{
		const auto weapons = pThis->Veterancy.IsElite() ? pTypeExt->EliteNewWeapons :
			pTypeExt->NewWeapons;

		int index = 0;
		if (pTechno->GetTechnoType()->WhatAmI() == AbstractType::InfantryType)
		{
			if (pTechno->IsInAir())
				index = pTypeExt->NewWeapon_Infantry_AIR.Get();
			else
				index = pTypeExt->NewWeapon_Infantry.Get();
		}
		else if (pTechno->GetTechnoType()->WhatAmI() == AbstractType::UnitType)
		{
			if (pTechno->IsInAir())
				index = pTypeExt->NewWeapon_Unit_AIR.Get();
			else
				index = pTypeExt->NewWeapon_Unit.Get();
		}
		else if (pTechno->GetTechnoType()->WhatAmI() == AbstractType::AircraftType)
		{
			if (pTechno->IsInAir())
				index = pTypeExt->NewWeapon_Aircraft_AIR.Get();
			else
				index = pTypeExt->NewWeapon_Aircraft.Get();
		}
		else
		{
			index = pTypeExt->NewWeapon_Building.Get();
		}
		...
		if (index > 0 && weapons.size() >= index)
		{
			const auto pNewWeapon = weapons[index - 1];
			if (pThis->DistanceFrom(pTarget) > pNewWeapon->MinimumRange && pExt->CheckSpecialWeapon(pTechno, pNewWeapon))
			{
		...
		{
			if (const auto pNewWeapon = weapons[index])
			{
				if (pThis->DistanceFrom(pTarget) > pNewWeapon->MinimumRange && pExt->CheckSpecialWeapon(pTechno, pNewWeapon))
				{
					pExt->TargetType = index + 1;
					return;
				}
				else
				{
...
bool TechnoExt::ExtData::CheckSpecialWeapon(TechnoClass* pTechno, WeaponTypeClass* pNewWeapon)
{
	const auto pThis = this->OwnerObject();
	...
	if (pTechno->IsInAir())
	{
		if (!pNewWeapon->Projectile->AA)
			return false;
	}
	else
	{
		if (!pNewWeapon->Projectile->AG)
			return false;
	}

	if (const auto pWarhead = pNewWeapon->Warhead)
	{
		auto armorType = pTechno->GetTechnoType()->Armor;

		if (const auto pTargetExt = TechnoExt::ExtMap.Find(pTechno))
		{
			if (const auto pShieldData = pTargetExt->Shield.get())
			{
				if (pShieldData->IsActive())
				{
					...
					armorType = pShieldData->GetType()->Armor.Get();
				}
			}
		}

		if (GeneralUtils::GetWarheadVersusArmor(pWarhead, armorType) == 0.0)
			return false;
		...
		{
			if (pTechno->GetTechnoType()->WhatAmI() != AbstractType::BuildingType)
				return false;
		}

		if (pNewWeapon->Warhead->MindControl)
		{
			if (pTechno->GetTechnoType()->ImmuneToPsionics || pTechno->IsMindControlled() ||
				pTechno->Owner == pThis->Owner || pThis->Owner->Allies.Contains(pTechno->Owner))
				return false;
		}
	}
