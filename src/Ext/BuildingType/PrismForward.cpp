...
bool BuildingTypeExt::cPrismForwarding::Load(PhobosStreamReader& Stm, bool RegisterForChange)
{
	return Stm
...
bool BuildingTypeExt::cPrismForwarding::Save(PhobosStreamWriter& Stm) const
{
	return Stm
		.Process(this->Enabled)
...
void BuildingTypeExt::cPrismForwarding::LoadFromINIFile(BuildingTypeClass* pThis, CCINIClass* pINI)
{
	...
	{
		if ((_strcmpi(Phobos::readBuffer, "yes") == 0) || (_strcmpi(Phobos::readBuffer, "true") == 0))
		{
			this->Enabled = EnabledState::Yes;
		}
		else if (_strcmpi(Phobos::readBuffer, "forward") == 0)
		{
			this->Enabled = EnabledState::Forward;
		}
		else if (_strcmpi(Phobos::readBuffer, "attack") == 0)
		{
			this->Enabled = EnabledState::Attack;
		}
		else if ((_strcmpi(Phobos::readBuffer, "no") == 0) || (_strcmpi(Phobos::readBuffer, "false")) == 0)
		{
			this->Enabled = EnabledState::No;
		}
	}
