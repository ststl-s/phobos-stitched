...
bool CrateClass::CanSpwan(CrateTypeClass* pType, CellClass* pCell)
{
	...
	bool occupied = false;
	auto cratelist = PhobosGlobal::Global()->Crate_List;
	auto it = std::find(cratelist.begin(), cratelist.end(), pCell);
	if (it != cratelist.end())
		occupied = true;

	if (!pCell->Passability ||
		(iswater && !pType->AllowSpwanOnWater) ||
		(!iswater && pType->AllowSpwanOnLand) ||
		occupied)
		return false;
...
void CrateClass::Update()
{
	...
	while (pObj)
	{
		if (const auto pBld = abstract_cast<BuildingClass*>(pObj))
		{
			this->IsInvalid = true;
			break;
		}
		else if(const auto pUnit = abstract_cast<UnitClass*>(pObj))
		{
		...
		else if (const auto pInf = abstract_cast<InfantryClass*>(pObj))
		{
			if ((!this->Type->AllowPick_InfantryTypes.empty() && !this->Type->AllowPick_InfantryTypes.Contains(pUnit->GetTechnoType())) ||
				(!this->Type->DisallowPick_InfantryTypes.empty() && this->Type->DisallowPick_InfantryTypes.Contains(pUnit->GetTechnoType())))
				continue;
		...
		for (int i = 0; i < num; ++i)
		{
			if (const auto pUnit = abstract_cast<TechnoClass*>(this->Type->Units[idx]->CreateObject(pOwner)))
			{
				...
				if (const auto pFoot = abstract_cast<FootClass*>(pUnit))
				{
					...
					pFoot->SetDestination(this->Location, true);
...
void CrateClass::CreateCrate(CrateTypeClass* pType, CellClass* pCell)
{
	...
	PhobosGlobal::Global()->Crate_List.emplace_back(crate, pCell);
}
